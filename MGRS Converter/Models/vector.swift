...
struct Vector: Equatable, CustomStringConvertible {
    ...
    func times(x: Double) -> Vector{
        return Vector(x: self.x * x, y: self.y * x, z: self.z * x)
    }
    
    func divideBy(x: Double) -> Vector{
        return Vector(x: self.x / x, y: self.y / y, z: self.z / z)
    }
    
    func dot(secondVector: Vector) -> Double{
        return self * secondVector
    }
    
    func cross(secondVector: Vector) -> Vector {
    ...
    func angleTo(vector1: Vector, planeNormal: Vector = Vector(x: 999, y: 999, z: 999)) -> Double{
    ...
    func rotateAround(axis: Vector, theta: Double) -> Vector{
        let p1 = self.unit()
